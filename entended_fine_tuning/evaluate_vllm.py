...
def main() -> int:
    ...
    for idx, record in enumerate(records, 1):
        try:
            pred, raw, n_tokens, latency = predict_one(base_url, args.model_id, record, args)
            status = "ok"
        ...
        logger.info(
            "Sample %d/%d complete | record_id=%s | status=%s | latency=%.3fs"
            " | generated_tokens=%d | tok/s=%.2f | pred_found=%s",
            idx,
            len(records),
