#!/usr/bin/env python3
"""Evaluate a locally fine-tuned LoRA adapter on the extended grounding task."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Evaluate local LoRA adapter for extended grounding")
    p.add_argument("--dataset", type=Path, default=Path("dataset.jsonl"))
    p.add_argument("--model-id", default=DEFAULT_MODEL)
    p.add_argument("--adapter", type=Path, required=True)
    p.add_argument("--output-dir", type=Path, default=Path("output/eval_lora"))
...
def setup_logging(log_file: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("extended_lora_eval")
...
def load_model(args: argparse.Namespace):
    tokenizer = AutoTokenizer.from_pretrained(args.adapter, use_fast=True, trust_remote_code=True)
    ...
    base = AutoModelForCausalLM.from_pretrained(args.model_id, **kwargs)
...
def main() -> int:
    ...
    if args.errors is None:
        args.errors = args.output_dir / "errors_lora.jsonl"
    if args.log_file is None:
        args.log_file = args.output_dir / "eval_lora.log"
