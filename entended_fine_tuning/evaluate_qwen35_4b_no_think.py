#!/usr/bin/env python3
"""Evaluate Qwen3.5-4B on the extended grounding task with thinking disabled."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--model-id", default=DEFAULT_MODEL, help="Hugging Face model id")
    p.add_argument("--output-dir", type=Path, default=Path("output/eval_qwen35_4b_no_think"))
...
def setup_logging(log_file: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("qwen35_4b_no_think_eval")
...
def render_prompt(tokenizer: AutoTokenizer, record: dict[str, Any], few_shot_path: Path) -> str:
    ...
    if hasattr(tokenizer, "apply_chat_template"):
        try:
            # Qwen chat templates use this flag to disable the explicit thinking mode.
            ...
            # Fallback for templates that do not expose enable_thinking but follow text instructions.
            return rendered + "\nDo not use thinking mode. Return the final JSON only.\n"
...
def main() -> int:
    ...
    if args.errors is None:
        args.errors = args.output_dir / "errors_qwen35_4b_no_think.jsonl"
    if args.log_file is None:
        args.log_file = args.output_dir / "eval_qwen35_4b_no_think.log"
